...
Coho.sub<- COHO %>% select(Brood.Year, Return.Year, Conservation.Unit, Tributary.Deme,Final.Spawner.Estimate, 
                           ...
                           Total.Fry,Total.Smolts, ER.Unclipped = ER.Rate...Unclipped,ER.Clipped = ER.Rate...Clipped,
                           Final.Fry.Survival = Final.Fry.Survival.using.Decision.Rules,
                           Final.Smolt.Survival = Final.Smolt.Survival.using.Decision.Rules,
...
Calc.Nat <- function(data=COHO.clean, CU="Middle Fraser", EC.max="NA", start.yr=1998, wsp.infill=TRUE, run.infill=FALSE, infill.start.yr=2014, WSP_only=TRUE){
  ...
  Ret.Hat = CUsums(data, CU, "Hatchery.Returns.corrected", EC.max, start.yr, wsp.infill, run.infill=FALSE, infill.start.yr, WSP_only)
...
BY.Table <- function(data, CU, EC.max="NA", start.yr, wsp.infill=TRUE, run.infill=FALSE, infill.start.yr=FALSE, WSP_only=TRUE){
              ...
              first <- start.data$Year[min(which(is.finite(start.data$Check)==TRUE))]
              ...
              sub <- start.data %>% rename(Brood_Year=Year) %>%
                             select(-Check, -Ret.Nat, -Ret.Hat,-Ret.Total) %>%
...
Run_write <- function(EC.max="NA", st.yr=1998,wsp.infill=TRUE,run.infill=FALSE, infill.start.yr=FALSE, WSP_only=TRUE){
                  ...
                  for(CU in CU.list){
                    start.yr <- st.yr
                    if(CU == "Fraser Canyon") { all.cus <- data.frame(matrix(NA, nrow=1,ncol=15))
                                                start.yr <- 1998  }
                              by.tbl <- BY.Table(data=COHO.clean, CU, EC.max, start.yr, wsp.infill, run.infill, infill.start.yr, WSP_only)
                              if(CU == "Fraser Canyon")  colnames(all.cus) <-colnames(by.tbl)
