...
fr.sk.bypop.raw <- read.csv("DATA_IN/SOURCES/Fraser Sockeye/Sockeye All (June 2020).csv",stringsAsFactors = FALSE)
# *********************************************************************************
...
write.csv(fr.sk.bypop.cleaned, "DATA_OUT/Cleaned_FlatFile_ByPop_FraserSockeye.csv",row.names=FALSE)
...
last.yr <- 2019 # last year for the merging and infilling
...
# ********************************************************************************************* BMAC ADDED 
CU.metrics.info <- read.csv("DATA_LOOKUP_FILES/MAIN_CU_LOOKUP_FOR_SOS.csv", stringsAsFactors = FALSE)
# ********************************************************************************************************
...
if(!dir.exists("DATA_OUT/FraserSockeyePrep")){dir.create("DATA_OUT/FraserSockeyePrep")}
...
test.out <- GetCUData(CU_Name = "Shuswap_L", Update = 2012, Data = "EFS",
                    ...
                    out.folder = "DATA_OUT/FraserSockeyePrep/") 
...
write.csv(as.matrix(cu.data.out), "DATA_OUT/Cleaned_FlatFile_ByCU_FraserSockeye.csv",row.names=FALSE)
